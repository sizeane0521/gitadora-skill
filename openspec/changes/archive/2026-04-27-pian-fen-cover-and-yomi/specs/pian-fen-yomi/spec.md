...
Each song row in the PianFen page SHALL display a gojuuon row badge (五十音行) indicating which GITADORA arcade TITLE folder the song belongs to. The badge SHALL appear inline with the difficulty, instrument, and level metadata. The badge text SHALL be one of: `あ行`, `か行`, `さ行`, `た行`, `な行`, `は行`, `ま行`, `や行`, `ら行`, `わ行`, or `A-Z`.
...
#### Scenario: ASCII or numeric song name displays A-Z

- **WHEN** a song name begins with an ASCII letter, digit, or symbol
- **THEN** the badge SHALL display `A-Z`
